...
class path_parse:
    ...
    def __init__(self,new_path=None,path_print=False):
        ...
        if(new_path != None):            
            self.path = new_path
            self.path_list = new_path.split("\\")
            self.path_head = new_path.split("\\")[0]
            ...
            self.path_head = self.path.split("\\")[0]
            self.path_contain = os.listdir(self.path)
            self.path_files = self.get_files()
            self.path_print = path_print
    ...
    def move_file(self,file_name,new_location,upbool = False):
        ...
        if(upbool == True):
            file_pathway = self.path+'\\'+file_name
        ...
        file_pathway = self.path+'\\'+file_name
        fold_loc = self.path+'\\'+new_location
    ...
    def cmd(self,motion):
        ...
        def cmd_cd(motion,nmot):
            ...
            if(dir_inst == '..'):  
                ...
                motion_test = self.path+"\\"+dir_inst
