# parsing_lib
# 
# Version 0.9 Construct
#
#
# List of functions contained in this file: number = 14
#
# __numeric_assert__
# __int_list__
# __empty_lists__
...
#
# Purpose: Returns error if n is not a numeric type, else nothing is returned
#
...
def numeric_uni_format(x):
    ...
    if(pos_bool is True):
        if(x < 100000000):
            outpt = round_decimal(x,7,True)
            for i in range(7):
                if(x > 10.0**(i+1)):
                    outpt = round_decimal(x,7-(i+1),True)
                    if(i == 6):
                        outpt = outpt+'.'
        if(x >= 100000000):
