...
def new_plot(x, y, label = None, lims = None, fontsize = 20, save = False, save_name = 'plot.jpg'):
    # ECC

    # Dummy Check for X and Y
    if(not __check__.array_test(x)):
        print("[new_plot] Error: 'x' must be an array")  
        return False    
    if(not __check__.array_test(y)):
        print("[new_plot] Error: 'y' must be an array") 
        return False      
    ...
    if(xarray == False and xnumeric == False):
        print("[new_plot] Error: input 'x' must either be a numeric array or an array of numeric arrays") 
    ...
    if(yarray == False and ynumeric == False):
        print("[new_plot] Error: input 'y' must either be a numeric array or an array of numeric arrays") 
                 
    if(yarray):
        if(len(x) != len(y)):
            print("[new_plot] Error: if 'y' is an array of numeric arrays, then it must have the same length as 'x'")
           
     
    if(not __check__.array_test(label)):
        label = [('',''),('','')]        

    if(__check__.array_test(lims)):
        lims_test = True
        for i in lims:
            if(not __check__.array_test(i)):
                lims_test = False
        if(lims_test):
            xlim = lims[0] 
            ylim = lims[1]
    ...
    if(len(x) != len(y)):
        newx = span_vec(x,len(y))
        spline_inst = spline() 
        try:
            newy = spline_inst.spline_val_scos(newx, x, y)
            x = newx 
            y = newy
        except:
            print("[new_plot] Error: 'x' and 'y' should be the same length")  
            return False   
        
    __plt__.plot(x, y)
    __plt__.ylabel(ylabel, fontsize = fontsize)
    __plt__.xlabel(xlabel, fontsize = fontsize)
    if(xlim != None):
...
def new_smooth_plot(x, y, label = None, smoothness = 300,
                    ylim = None, xlim = None, save = False, save_name = 'plot.jpg'):
      
    spln_inst = spline(x, y)
    xsmooth = span_vec(x, smoothness)
    spln_inst.pass_newx_vec(xsmooth)  
