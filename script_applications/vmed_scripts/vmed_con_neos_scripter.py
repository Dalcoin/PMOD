...
success, in_path_list = cml.cmd("dir test_input.txt")
if(success == False):
    print("[vmed_con_eos_scripter] Error: 'test_input.txt' could not be accessed")
    exit_func()
...
if(demarc.rstrip() != "end param."):
    print("[vmed_con_eos_scripter] Warning: the end line in 'test_input.txt' is not 'end param.'")
...
#    subp.call("./neos", shell=True)
    ...
    # Consolidating Files
    cml.cmd('ls')
    cml.cmd('mv output.don fort.12 vincp')
    cml.cmd('cd vincp')
    new_fold_name = 'run_'+str(numgroup[i])
    cml.cmd('mkdir '+new_fold_name)
    cml.cmd('mv output.don fort.12 '+new_fold_name)
    cml.cmd('cd ..')
